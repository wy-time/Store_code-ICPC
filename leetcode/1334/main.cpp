...
// }
unsigned int dis[105][105];
int findTheCity(int n, vector<vector<int>>& edges, int distanceThreshold) {
    memset(dis,0x11,sizeof(unsigned int) * 10000);
    ...
    int ans;
    int minCount = 0x7f7f7f7f;
    for (int i = 0; i<n; i++) {
        int count = 0;
        for (int j = 0; j<n; j++) {
            count += (dis[i][j] <= distanceThreshold);
        }
        if(count <= minCount) {
            minCount = count;
            ans = i;
...
int main()
{
    ...
    cout<<findTheCity(n,edges,6586)<<endl;
    // wfor(i,0,n) {
    //     int val;
    //     cin>>val;
    //     nums.push_back(val);
